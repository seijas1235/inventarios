var tiposcliente_table = $('#tiposcliente-table').DataTable({
       ...
       
    {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a href='/tipos_cliente/edit/"+ full.id +"' class='edit-tipocliente'>" + 
...
$('body').on('click', 'a.remove-tipocliente', function(e) {
    ...
    if (user.length = 1) {
        $("#message").text("este tipo de cliente?");
        $(".variable").text("");
        $(".entity").text("");
    } else {
        $("#message").text("este tipo de cliente");
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/tipos_cliente/remove/"+id;
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Tipo de cliente eliminado exitosamente!");
        tiposcliente_table.ajax.reload();
