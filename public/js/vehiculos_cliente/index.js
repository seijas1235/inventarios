var vehiculos_cliente_table = $('#vehiculos_cliente-table').DataTable({
    "ajax": "/vehiculos_cliente/getJson",
    "responsive": true,
    "processing": true,
    "serverSide": true,
    "info": true,
    ...
    },       
    {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a href='/vehiculos_cliente/edit/"+ full.id +"' class='edit-vehiculocliente'>" + 
            "<i class='fa fa-btn fa-edit' title='Editar asignacion'></i>" + 
            "</a>" + "</div>" + 
            "<div class='float-right col-lg-6'>" + 
            "<a href='#' class='remove-vehiculocliente'>" + 
            "<i class='fa fa-btn fa-trash' title='Eliminar asignacion'></i>" + 
            "</a>" + "</div>";
...
$('body').on('click', 'a.remove-vehiculocliente', function(e) {
    ...
    if (user.length = 1) {
        $("#message").text("esta asiganacion de vehiculo?");
        $(".variable").text("");
        $(".entity").text("");
    } else {
        $("#message").text("esta asiganacion de vehiculo");
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/vehiculos_cliente/remove/"+id;
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Asignacion eliminada exitosamente!");
        vehiculos_cliente_table.ajax.reload();
