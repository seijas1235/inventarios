...
function saveVale() {
	...
	var no_vale = $("#no_vale").val();

	var formData = {
		no_vale: no_vale, total_vale: total_vale, bomba_id: bomba_id, cliente_id: cliente_id, detalle : db.links, piloto: piloto, placa: placa
	} 
	$.ajax({
		type: "GET",
		url: "/sfi/vales/save",
...
$("#SelecCliente").click(function(e) {
	...
	if (cliente_id && no_vale)  {
		...
		bootbox.alert("Debe de seleccionar un cliente y un No. de vale");
	}
});
...
$(".gasolinaButton").click(function(e) {
	...
	$.ajax({
		...
		error: function() {
			alert("No se pudo crear la orden, verifique su conexi¨®n");
		}
	});
