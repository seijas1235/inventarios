    ...
    "columns": [ {
        ...
        "render": function(data, type, full, meta) {
            if (full.estado_empleado != "Inactivo") {
               ...
               "<a href='/sfi/empleados/edit/"+ full.id +"' class='edit-empleado'>" + 
               "<i class='fa fa-btn fa-edit' title='Editar'></i>" + 
               "</div>" + "<div class='float-right col-lg-6'>" + 
               "<a href='#' class='remove-empleado'>" + 
            ...
            return "<div id='" + full.id + "' class='text-center'>" + 
            "<div class='float-left col-lg-6'>" + 
            "<a href='#' class='activar-cliente'>" + 
            "<i class='fa fa-btn fa-check' title='Activar'></i>" + 
            "</a>" + 
            "</div></div> ";
        }
...
$('body').on('click', 'a.activar-cliente', function(e) {
    $( ".confirm-active" , "#userActiveModel").removeAttr("field");
    var id = $(this).parent().parent().attr("id");
    $("input[name='password_delete_a']").val("");
    unsetPasswordErrors("password_delete_a");
    var user = $(this).parent().parent().parent().parent().children().eq(0).text();
    $("#userActiveModel").hide().show();
    $("#userActiveModel").modal();
    if (user.length = 1) {
    ...
    $(".confirm-active", "#userActiveModel").attr("id", "active-" + id);
});
...
$('body').on('click', 'button.confirm-active', function( e ) {
    ...
    var url = "/sfi/empleado/active/"+id;
    var password_delete_a = $("input[name='password_delete_a']").val().trim();
    data = {
        password_delete_a : password_delete_a
    };
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Empleado activado exitosamente!");
        empleado_table.ajax.reload();
        $("#userActiveModel").modal("hide");
    }).fail(function(errors) {
        var errors = JSON.parse(errors.responseText);
        if (errors.password_delete_a != null) setPassworddErrors("password_delete_a", errors.password_delete_a);
        else unsetPasswordErrors("password_delete_a");
    });
    return false;
});
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/sfi/empleado/remove/"+id;
