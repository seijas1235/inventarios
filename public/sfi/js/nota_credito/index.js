var nota_credito2_table = $('#nota_credito2-table').DataTable({
    "ajax": "/sfi_tecu/nota_credito2/getJson",
    "responsive": true,
    "processing": true,
    "serverSide": true,
    "info": true,
    ...
    "columns": [ {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a href='/sfi_tecu/nota_credito2/edit/"+ full.id +"' class='edit-nota_credito'>" + 
            "<i class='fa fa-btn fa-edit' title='Editar'></i>" + 
            "</div>" + "<div class='float-left col-lg-6'>" + 
            "<a href='#' class='remove-nota_credito2'>" + 
...
$('body').on('click', 'a.remove-nota_credito2', function(e) {
    ...
    if (user.length = 1) {
        $("#message").text("esta nota de crédito?");
        $(".variable").text("");
        $(".entity").text("");
    } else {
        $("#message").text("esta nota de crédito");
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/sfi_tecu/nota_credito2/remove/"+id;
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Nota crédito borrada exitosamente!");
        nota_credito_table.ajax.reload();
