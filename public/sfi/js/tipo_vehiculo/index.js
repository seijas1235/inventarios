var tipo_vehiculo_table = $('#tipo_vehiculo-table').DataTable({
    "ajax": "/sfi/tipo_vehiculo/getJson",
    "responsive": true,
    "processing": true,
    "serverSide": true,
    "info": true,
    ...
    "columns": [ {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a href='/sfi/tipo_vehiculo/edit/"+ full.id +"' class='edit-tipo_vehiculo'>" + 
            "<i class='fa fa-btn fa-edit' title='Editar'></i>" + 
            "</div>" + "<div class='float-left col-lg-6'>" + 
            "<a href='#' class='remove-tipo_vehiculo'>" + 
...
$('body').on('click', 'a.remove-tipo_vehiculo', function(e) {
    ...
    if (user.length = 1) {
        $("#message").text("este tipo de vehiculo?");
        $(".variable").text("");
        $(".entity").text("");
    } else {
        $("#message").text("este tipo de vehiculo");
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/sfi/tipo_vehiculo/remove/"+id;
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Tipo de Vehiculo borrado exitosamente!");
        tipo_vehiculo_table.ajax.reload();
