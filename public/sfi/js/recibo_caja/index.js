var vale_table = $('#vale-table').DataTable({
    ...
    "columns": [ {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a target='_blank' href='/sfi/recibo_caja/show/"+ full.id +"' class='detalle-vale'>" + 
            "<i class='fa fa-btn fa-file-archive-o' title='Detalle'></i>" + 
            "</a>" + "</div>"
            + "<div class='float-right col-lg-6'>" + 
            "<a href='#' class='remove-producto'>" + 
            "<i class='fa fa-btn fa-trash' title='Anular Recibo'></i>" + 
            "</a>" + "</div>";
            }
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/sfi/recibo_caja/remove/"+id;
