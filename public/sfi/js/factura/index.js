var factura1_table = $('#factura1-table').DataTable({
    "ajax": "/sfi_tecu/factura/getJson",
    "responsive": true,
    "processing": true,
    "serverSide": true,
    "info": true,
    ...
    "columns": [ {
        ...
        "render": function(data, type, full, meta) {
                ...
                "<a target='_blank' href='/sfi_tecu/factura/show/"+ full.id +"' class='generar-factura'>" + 
                "<i class='fa fa-btn fa-file-archive-o' title='Generar Factura'></i>" + 
                "</a>" + "</div>"
                + "<div class='float-right col-lg-6'>" + 
                "<a href='#' class='remove-producto'>" + 
                "<i class='fa fa-btn fa-trash' title='Anular Factura'></i>" + 
                "</a>" + "</div>";    
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/sfi_tecu/factura/remove/"+id;
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Factura anulada exitosamente!");
        factura1_table.ajax.reload();
