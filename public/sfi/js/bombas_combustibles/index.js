var bombacombustible_table = $('#bombacombustible-table').DataTable({
    "ajax": "/sfi/bombas_combustibles/getJson",
    "responsive": true,
    "processing": true,
    "serverSide": true,
    "info": true,
    ...
    "columns": [  {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a href='/sfi/bombas_combustibles/edit/"+ full.id +"' class='edit-bomba-combustible'>" + 
            "<i class='fa fa-btn fa-edit' title='Editar'></i>" + 
            "</a>" + "</div>" + 
            "<div class='float-right col-lg-3'>" + 
            "<a href='#' class='remove-bomba_combustible'>" + 
            "<i class='fa fa-btn fa-trash' title='Eliminar'></i>" + 
            "</a>" + "</div>";
        },
    ...
    "createdRow": function(row, data, rowIndex) {
        $.each($('td', row), function(colIndex) {
            if (colIndex == 1) $(this).attr('id', data.id);
        });
    },
...
$('body').on('click', 'a.remove-bomba_combustible', function(e) {
    ...
    if (user.length = 1) {
        $("#message").text("esta bomba y combustible?");
        $(".variable").text("");
        $(".entity").text("");
    } else {
        $("#message").text("esta bomba y combustible");
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    var url = "/sfi/bombas_combustibles/remove/"+id;
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("Bomba y Combustible eliminada exitosamente!");
        bombacombustible_table.ajax.reload();
