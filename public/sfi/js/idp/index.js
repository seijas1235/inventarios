var idp_table = $('#idp-table').DataTable({
    "ajax": "/sfi/idp/getJson",
    "responsive": true,
    "processing": true,
    "serverSide": true,
    "info": true,
    ...
    "columns": [  {
        ...
        "render": function(data, type, full, meta) {
            ...
            "<a href='/sfi/idp/edit/"+ full.id +"' class='edit-idp'>" + 
            "<i class='fa fa-btn fa-edit' title='Editar IDP'></i>" + 
            "</div>"  + "<div class='float-right col-lg-6'>" + 
            "<a href='#' class='remove-idp'>" + 
            "<i class='fa fa-btn fa-trash' title='Eliminar IDP'></i>" + 
            "</a>" + "</div>" + "</div>";
        },
...
$('body').on('click', 'a.remove-idp', function(e) {
...
$('body').on('click', 'button.confirm-delete', function( e ) {
    ...
    $.ajax({
        ...
        $(".user-created-message > p").text("IDP borrada exitosamente!");
        idp_table.ajax.reload();
